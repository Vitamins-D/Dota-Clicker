...
LinkLuaModifier( "modifier_pudge_meat_hook_followthrough_lua", "abilities/pudge/modifier_pudge_meat_hook_followthrough_lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_pudge_meat_hook_lua", "abilities/pudge/modifier_pudge_meat_hook_lua", LUA_MODIFIER_MOTION_HORIZONTAL )
...
function pudge_meat_hook_lua:OnSpellStart()
	...
	self.distance_damage = self:GetSpecialValueFor( "distance_to_damage" )
	self.hook_followthrough_constant = self:GetSpecialValueFor( "hook_followthrough_constant" )
	self.distance_damage = self.distance_damage/100
		...
		if hTarget ~= nil then
			...
			if hTarget:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
				...
				if self.vStartPosition then
					dist = (self.vStartPosition - hTarget:GetAbsOrigin()):Length2D()
				end
				local damage = {
						victim = hTarget,
						attacker = self:GetCaster(),
						damage = self.hook_damage+dist*self.distance_damage,
