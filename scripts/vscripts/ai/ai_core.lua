...
function AICore:getEnemies(radius, tEntity, minDist)
	local enemies = FindUnitsInRadius(
        ...
        DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO, -- Все юниты кроме построек
...
function AICore:getAllEnemies(radius, tEntity, minDist)
	local enemies = FindUnitsInRadius(
        ...
        radius,
        DOTA_UNIT_TARGET_TEAM_ENEMY,
        DOTA_UNIT_TARGET_BASIC+ DOTA_UNIT_TARGET_BASIC , -- Все юниты кроме построек
...
function AICore:getAllies(radius, tEntity)
	local allies = FindUnitsInRadius( -- Переименована переменная: было enemies, теперь allies
		...
		DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_BASIC, -- Добавлены герои и крипы
        DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE,
        FIND_CLOSEST,
        false
    )
